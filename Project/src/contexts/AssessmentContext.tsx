...
interface AssessmentContextType {
  ...
  calculateResult: (cognitive?: CognitiveData, ayurvedic?: AyurvedicData) => void;
...
export const AssessmentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const calculateResult = (cognitive?: CognitiveData, ayurvedic?: AyurvedicData) => {
    const cogData = cognitive || cognitiveData;
    const ayuData = ayurvedic || ayurvedicData;
    
    if (!cogData || !ayuData) return;

    // Cognitive risk calculation
    const cognitiveScore = (
      cogData.memoryIssues +
      cogData.concentrationDifficulty +
      cogData.languageProblems +
      cogData.orientationConfusion +
      cogData.dailyActivityImpairment +
      (5 - cogData.sleepQuality) +
      cogData.stressLevel +
      (5 - cogData.physicalActivity) +
      (5 - cogData.socialEngagement) +
      cogData.moodChanges
    ) / 10;
    ...
    const responses = [
      ...
      ayuData.appetite,
      ayuData.digestion,
      ayuData.sleepPattern,
      ayuData.energyLevel,
      ayuData.temperament,
      ayuData.stressResponse,
      ayuData.weatherPreference
    ];
